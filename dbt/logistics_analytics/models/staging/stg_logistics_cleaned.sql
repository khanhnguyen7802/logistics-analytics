...
-- Deduplicate: keep only the most recent record per booking_id
deduped as (
    select 
        *,
        row_number() over (partition by booking_id order by booking_date, trip_start_date desc) as rn
    from stg
),
...
cleaned as (
    ...
    from deduped
    where rn = 1  -- Keep only the most recent record per booking_id
)
